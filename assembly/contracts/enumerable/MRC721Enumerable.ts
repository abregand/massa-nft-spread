...
import { _isOwner, _setOwner } from '../utils/ownership-internal';
import { Context, isDeployingContract } from '@massalabs/massa-as-sdk';
import { isMintable, isTransferable } from '../manageable/manageable';
 ...
 */
export function mrc721Constructor(name: string, symbol: string, maxSupply: u256): void {
  assert(isDeployingContract());
  _constructor(name, symbol, maxSupply);
...
export function transferFrom(binaryArgs: StaticArray<u8>): void {
  if(!_isOwner(Context.caller().toString())) {
    assert(isTransferable(), 'NFT is not transferable');
  }
 ...
 *
 * @remarks This function is only callable if the contract is mintable or the caller is the owner of the contract.
 */
export function mint(binaryArgs: StaticArray<u8>): void {
  assert(_totalSupply() < _maxSupply(), 'Max supply reached');
  if(!_isOwner(Context.caller().toString())) {
    assert(isMintable(), 'NFT is not mintable');
    assert(Context.transferredCoins() >=  _mintPrice(), 'transferred coins not cover the mint fee');
  }
  const args = new Args(binaryArgs);
  const to = args.nextString().expect('to argument is missing or invalid');
  const tokenId = args
    .nextU256()
    .expect('tokenId argument is missing or invalid');
  assert(tokenId <= _maxSupply(), 'tokenId exceeds max supply');
  _update(to, tokenId, '');
}
...
/**
 * Returns the max number of tokens.
 * @returns a serialized u256 representing the total supply
 */
export function maxSupply(_: StaticArray<u8>): StaticArray<u8> {
  return u256ToBytes(_maxSupply());
}
