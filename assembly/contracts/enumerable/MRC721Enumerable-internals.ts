...
import { bytesToU256, bytesToU64, stringToBytes, u256ToBytes, u64ToBytes } from '@massalabs/as-types';
import {
  _isAuthorized,
  _ownerOf,
  ownerKey,
...
export const TOTAL_SUPPLY_KEY: StaticArray<u8> = stringToBytes('TOTAL_SUPPLY');
export const MAX_SUPPLY_KEY: StaticArray<u8> = stringToBytes('MAX_SUPPLY');
export const OWNED_TOKENS_KEY: StaticArray<u8> = stringToBytes('OWNED_TOKENS_');
export const MINT_PRICE_KEY: StaticArray<u8> = stringToBytes('MINT_PRICE');
 ...
 */
export function _constructor(name: string, symbol: string, maxSupply: u256): void {
...
/**
 * Returns the max number of tokens possible.
 */
export function _maxSupply(): u256 {
  return bytesToU256(Storage.get(MAX_SUPPLY_KEY));
}

/**
 * Returns the mint price for each token.
 */
export function _mintPrice(): u64 {
  return bytesToU64(Storage.get(MINT_PRICE_KEY));
}

/**
 * Set the mint price for each token.
 * @param price
 */
export function _setMintPrice(price: u64): void {
